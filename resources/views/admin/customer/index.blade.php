                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $customer->phone_number }}</td>
                                               <td> <button class="btn btn-success" name="edit_button"
                                                    value="{{ $customer->id }}" data-toggle="modal"
                                                    data-target="#edit_modal"><i class="fa fa-edit"></i></button>
                                                <button class="btn btn-danger mr-2"
                                                    onclick="if(confirm('هل أنت متأكد ؟')){document.getElementById('delete-users_{{ $customer->id }}').submit();}else{
                                            event.preventDefault();}"><i
                                                        class="fa fa-trash"></i></button>
                                                <form id="delete-users_{{ $customer->id }}"
                                                    action="/admin/customer/{{ $customer->id }}" method="POST"
                                                    class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form form-vertical" action="/admin/customer" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-body">
                            <div class="row">
                                <div class="col-6">
    ...
    <script>
        $(document).ready(function() {
            $("button[name='edit_button']").click(function() {
                ...
                $.get("/admin/customer/" + edit_val + "/edit", function(data, status) {
